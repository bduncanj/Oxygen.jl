...
function registerschema(
    ...
    return_types = []
    for item_type in returntype
        # Exclude the base subtype which would otherwise match everything
        if item_type === Union{} 
            continue
        end
        # A type wrapper has captured the return type so extract it
        if item_type <: ResponseWrapper
            item_type = item_type.parameters[1]  
        end
        
        # Structs from handlers can return as ::Union{T,Nothing}
        union_info = extract_concrete_types(item_type)
        if !isnothing(union_info)
            # Extract collection of non-null types from this Union
            _, inner_types = union_info
            append!(return_types, inner_types)
        else
            push!(return_types, item_type)
        end
    end
    ...
    exclude_types = Set([HTTP.Messages.Response, Any])
    filtered = filter(x -> x ∉ exclude_types, return_types);
...
function redochtml(schemapath::String, docspath::String) :: ResponseWrapper
...
"""
function swaggerhtml(schemapath::String, docspath::String) :: ResponseWrapper
